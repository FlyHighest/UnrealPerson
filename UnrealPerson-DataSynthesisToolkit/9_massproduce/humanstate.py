...
class HumanState():
    ...
    def _getAllowedClothesForPartAndGender(self,part,gender):
        ...
        if part=='Full' or part=='Upper' or part=='Lower':
            part = part + "Clothes"
        ...
        assert len(allowedPaths)>0, "Part:{} Gender:{}  ---  No clothes available".format(part,gender)
        return allowedPaths
    ...
    def _randomizeFulloClothes(self):
        allowed = self._getAllowedClothesForPartAndGender("fullo", self._getCurrentGender())
        picked = self._pickOneFromArray(allowed)
        self.clothes.append(picked)

    def _randomizeHandheldClothes(self):
        allowed = self._getAllowedClothesForPartAndGender("handheld", self._getCurrentGender())
        picked = self._pickOneFromArray(allowed)
        self.clothes.append(picked)

    def _randomizeHeadwearClothes(self):
        allowed = self._getAllowedClothesForPartAndGender("headwear", self._getCurrentGender())
        picked = self._pickOneFromArray(allowed)
        self.clothes.append(picked)

    def _randomizeScarfClothes(self):
        allowed = self._getAllowedClothesForPartAndGender("scarf", self._getCurrentGender())
        picked = self._pickOneFromArray(allowed)
        self.clothes.append(picked)

    def _randomize_blank(self, ind):
        allowed = self._getAllowedClothesForPartAndGender("blank", self._getCurrentGender())
        self.clothes.append(allowed[ind])

    def _randomizeMaskClothes(self):
        allowed = self._getAllowedClothesForPartAndGender("mask", self._getCurrentGender())
        picked = self._pickOneFromArray(allowed)
        self.clothes.append(picked)

    def _randomizeBackpackClothes(self):
        allowed = self._getAllowedClothesForPartAndGender("backpack", self._getCurrentGender())
        picked = self._pickOneFromArray(allowed)
        self.clothes.append(picked)

    def _randomizeGlassesClothes(self):
        allowed = self._getAllowedClothesForPartAndGender("glasses", self._getCurrentGender())
        picked = self._pickOneFromArray(allowed)
        self.clothes.append(picked)
    ...
    def _randomizeProxies(self):
        ...
        # This place controls how a human is produced , controls the variaty of foreground
        # Setting v1: Up+low+shoe
        # Setting v2: See excel;
        # full.(30 %)
        # up + low(60 %)
        # fullo + up + low(10 %)
        # shoe
        ...
        if random_num <= 0:
            ...
            self._randomize_blank(blank_ind)
            blank_ind += 1
            self._randomize_blank(blank_ind)
            blank_ind += 1
            self._randomize_blank(blank_ind)
        ...
        num_acc = 0
        random_num = random.randint(1,100)
        if random_num <= 30 and num_acc<4:
            self._randomizeBackpackClothes()
            num_acc += 1
        else:
            self._randomize_blank(blank_ind)
            blank_ind += 1
        ...
        random_num = random.randint(1,100)
        if random_num <= 30 and num_acc<4:
            self._randomizeHeadwearClothes()
            num_acc += 1
        else:
            self._randomize_blank(blank_ind)
            blank_ind += 1
        ...
        random_num = random.randint(1,100)
        if random_num <= 20 and num_acc<4:
            self._randomizeGlassesClothes()
            num_acc += 1
        else:
            self._randomize_blank(blank_ind)
            blank_ind += 1
        ...
        random_num = random.randint(1,100)
        if random_num <= 30 and num_acc<4:
            self._randomizeHandheldClothes()
            num_acc += 1
        else:
            self._randomize_blank(blank_ind)
            blank_ind += 1
        ...
        random_num = random.randint(1,100)
        if random_num <= 5 and num_acc<4:
            self._randomizeScarfClothes()
            num_acc += 1
        else:
            self._randomize_blank(blank_ind)
            blank_ind += 1
    ...
    def equipClothes(self):
        ...
        count=0
        while count<3: #len(mhapi.assets.getEquippedClothes())!=len(self.clothes):

            print("equip {}".format(count))
